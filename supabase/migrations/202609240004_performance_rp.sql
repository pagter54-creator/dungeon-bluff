begin;
-- Pure server-owned calculation. Tied placement uses the occupied ranks' mean
-- baseline, then the protection/cap of the shared rank (competition ranking).
create or replace function public.account_rp_delta(p_score numeric,p_average numeric,p_gold numeric,p_rank integer,p_ties integer)
returns integer language plpgsql immutable set search_path='' as $$
declare base numeric; delta integer; begin
 if p_rank not between 1 and 4 or p_ties<1 or p_rank+p_ties-1>4 then raise exception 'Invalid placement'; end if;
 select avg((array[30,10,-10,-20])[i]) into base from generate_series(p_rank,p_rank+p_ties-1) i;
 delta:=round(base+(p_score-p_average)*0.35+greatest(0,p_gold)*0.1)::integer;
 return case p_rank when 1 then greatest(10,least(100,delta)) when 2 then greatest(0,least(60,delta)) when 3 then greatest(-70,least(0,delta)) else greatest(-100,least(-5,delta)) end;
end $$;
revoke all on function public.account_rp_delta(numeric,numeric,numeric,integer,integer) from public,anon,authenticated;
grant execute on function public.account_rp_delta(numeric,numeric,numeric,integer,integer) to service_role;
...
declare m record; member jsonb; source jsonb; n integer; ties integer; delta integer; score integer; gold integer; old_rating integer; new_rating integer; percent integer; raw_score integer; raw_gold integer; cleared boolean; begin
 ...
 for m in select r.member_id,r.user_id from public.game_reward_members r join public.room_members rm on rm.id=r.member_id and rm.user_id=r.user_id join public.profiles pr on pr.user_id=r.user_id where r.session_id=new.id and pr.account_type='registered' order by r.user_id loop
  ...
  delta:=0;
  if cleared then
   select public.account_rp_delta(score,avg((value->>'score')::numeric),gold,n,ties) into delta from jsonb_each(new.state->'players');
  end if;
