...
export function partyPanels(bundle, players, { me, result, selected, useSkill }) {
  ...
  return [...bundle.members].sort((a,b) => a.seat_index-b.seat_index).map(m => {
    ...
    return `<article class="player-panel illustrated-panel seat-${m.seat_index} ${own ? 'is-me' : ''} ${p.knockedOut ? 'knocked-out' : ''}" data-player="${m.id}" style="--character-color:${html(c.definition?.color || '#dabc85')}">
      <div class="player-art-stage">${skinIllustration(p.characterId||c.id,p.loadout)}</div>
      <div class="player-info">
        ...
        <div class="player-bottom">${skillBadge(c)}<span class="lock-state ${ready?'ready':''}">${result ? '공개 중' : seen ? `선택: ${seen.value}` : p.knockedOut ? '자동 제출' : ready ? '✓ 선택 완료' : '선택 중'}</span></div>
        ${greed?'<div class="boss-player-mark">탐욕 표식 · 다음 기본 공격 대상</div>':''}${marked ? `<div class="seer-vision">✧ ${publicMark?'표적 지정 · 전체 공개':'계시 대상'} · ${seen ? `선택: <b>${seen.value}</b>` : '제출을 기다리는 중'}</div>` : ''}${own ? activeButton(p,useSkill,ready || p.knockedOut) : ''}${own ? panelControls(p,{result,locked:ready,selected,useSkill,twoCards:isShuffleTurn(bundle.session)}) : ''}${p.skillId==='toughness' ? `<small class="runtime-label">강인함 ${p.characterRuntimeState.toughnessAvailable?'READY':'소진 · HP 2 이상 회복 시 충전'}</small>` : ''}
      </div>
    </article>`;
  }).join('');
}
