...
export function deckLabel(character) {
  if (character.definition?.deckType === 'continuous') return '1~7 랜덤 · 손패 2장 · 매 턴 보충';
...
export function characterChoices(bundle, memberId) {
  return `<div class="eyebrow">CHOOSE YOUR CHARACTER</div><h2>당신의 패, 당신의 방식.</h2><p>같은 캐릭터도 함께 선택할 수 있습니다.</p><div class="character-choices">${(bundle.characters || []).map(c => `<button class="character-choice" data-action="set-character" data-member="${html(memberId)}" data-character="${html(c.id)}" data-network style="--character-color:${html(c.definition.color)}"><span class="character-icon">${skinPortrait(c.id)}</span><span><b>${html(c.display_name)}</b><small>${html(c.definition.role)}</small><span class="character-deck">${html(deckLabel(c))}</span><strong>${c.definition.skill.type.toUpperCase()} · ${html(c.definition.skill.name)}</strong><p>${html(c.definition.skill.description)}</p></span></button>`).join('')}</div>`;
}
export function cardPool(player, { own=false, blocked=false, selected=null } = {}) {
  return `<div class="cycle-pool ${player.character?.definition?.deckType==='continuous'?'continuous-hand':''}" data-cycle-pool="${html(player.memberId)}" aria-label="현재 손패 ${cycleCards(player).length}장">${cycleCards(player).map(c=>cardComponent(c,{loadout:player.loadout,own,blocked,selected})).join('')}</div>`;
}
...
export function partyPanels(bundle, players, { me, result, selected, useSkill }) {
  ...
  return [...bundle.members].sort((a,b) => a.seat_index-b.seat_index).map(m => {
    ...
    return `<article class="player-panel illustrated-panel seat-${m.seat_index} ${own ? 'is-me' : ''} ${p.knockedOut ? 'knocked-out' : ''}" data-player="${m.id}" style="--character-color:${html(c.definition?.color || '#dabc85')}">${skinIllustration(p.characterId||c.id,p.loadout)}<div class="player-heading"><div class="hearts" aria-label="HP ${p.hp}/${p.maxHp}">${Array.from({length:p.maxHp},(_,i)=>`<span class="heart ${i<p.hp?'filled':''}">♥</span>`).join('')}</div></div><div class="player-content"><div class="player-stats"><span>SCORE <b>${p.score}</b></span><span>RUN GOLD <b>${p.gold}</b></span><small>${c.definition?.deckType==='continuous'?'운명의 패':`CYCLE ${p.cycleIndex || 1}`} · ${cycleCards(p).filter(card=>!card.used).length}장 남음</small></div>${cardComponent(null,{loadout:p.loadout,blocked:ready,revealId:m.id})}</div>${cardPool(p,{own,blocked:ready || p.knockedOut,selected})}<div class="player-identity player-identity-bar" title="${html(m.display_name)}"><h3>${html(m.display_name)} ${own ? '<em>나</em>' : ''}</h3><small>${html(c.display_name)}${m.ai_type ? ' · AI' : ''}${p.knockedOut ? ' · 기절' : ''}</small></div><div class="player-bottom">${skillBadge(c)}<span class="lock-state ${ready?'ready':''}">${result ? '공개 중' : seen ? `선택: ${seen.value}` : p.knockedOut ? '자동 제출' : ready ? '✓ 선택 완료' : '선택 중'}</span></div>${greed?'<div class="boss-player-mark">탐욕 표식 · 다음 기본 공격 대상</div>':''}${marked ? `<div class="seer-vision">✧ ${publicMark?'표적 지정 · 전체 공개':'계시 대상'} · ${seen ? `선택: <b>${seen.value}</b>` : '제출을 기다리는 중'}</div>` : ''}${own ? activeButton(p,useSkill,ready || p.knockedOut) : ''}${own ? panelControls(p,{result,locked:ready,selected,useSkill,twoCards:isShuffleTurn(bundle.session)}) : ''}${p.skillId==='toughness' ? `<small class="runtime-label">강인함 ${p.characterRuntimeState.toughnessAvailable?'READY':'소진 · HP 2 이상 회복 시 충전'}</small>` : ''}</article>`;
  }).join('');
}
export function panelControls(player,{result,locked,selected,useSkill,twoCards=false}) {
 ...
 const label=twoCards?`뒤죽박죽 · ${info.cards.length}/${info.count}장 선택 · 무작위 1장 소비`:(info.cards[0]?info.cards[0].value+' 선택'+(useSkill?' · 증폭 +2':''):'카드를 선택하세요');
