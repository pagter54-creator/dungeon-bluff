...
document.addEventListener('click', async event => {
  ...
  if (action === 'select-card' && !animating) { selected = button.dataset.cardId; renderGame(); }
  if (action === 'submit' && selected !== null && !animating) {
    const me = mine(), g = bundle.session;
    const card = cycleCards(g.state.players[me.id]).find(c => c.id === selected && !c.used);
    if (card) void perform('submit_card', { session_id: g.id, turn_index: g.turn_index, member_id: me.id, card_id: card.id, card_value: card.value, use_skill: useSkill });
  }
});
