...
export function cardComponent(card, {loadout,own=false,blocked=false,selected=null,faceDown=false,revealId=null}={}){
 ...
 return `<button type="button" class="pool-card ${front} ${back} ${card.used?'spent used':''} ${own&&chosen?'chosen selected':''} ${blocked?'locked':''} ${faceDown?'face-down':''}" data-card-instance="${esc(card.id)}" ${own?`data-action="select-card" data-card-id="${esc(card.id)}"`:''} ${disabled?'disabled':''} aria-pressed="${own&&chosen}" aria-label="${faceDown?'비공개 카드':card.value}, ${card.used?'사용 완료':'사용 가능'}"><b>${faceDown?'◇':card.value}</b><small>${card.used?'OFF':'◆'}</small></button>`;
}
