...
import {skinFor,pendingSkinImage} from './skins.js';
...
export function entryAssetPlan(session){
 ...
 return {required:[...new Set([...skins.map(s=>s.preview),...ids.map(id=>EVENT_IMAGES[id]||MONSTER_IMAGES[shapes[id]])].filter(Boolean))],poses:[...new Set(skins.flatMap(s=>[s.attack,s.damage]))]};
}
...
export function warmLobbyAssets(members){
 ...
 const work=[...skins.flatMap(s=>[[s.preview,false],[s.attack,true],[s.damage,true]]),...Object.values(MONSTER_IMAGES).concat(Object.values(EVENT_IMAGES)).map(url=>[url,false])];
