...
export async function flipRevealCard(card, result, reduced=false) {
  if(!card)return;
  if(!reduced)await finishAnimation(card.animate([{transform:'perspective(650px) translateY(0) rotateY(0)'},{transform:'perspective(650px) translateY(-14px) rotateY(88deg)'}],{duration:210,easing:'ease-in'}));
